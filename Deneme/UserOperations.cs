            ...
            for (int i = 0; i < 100; i++)
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        {
                            ExecuteUpdateQuery(cmd, "20110101", "20111231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteUpdateQuery(cmd, "20120101", "20121231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteUpdateQuery(cmd, "20130101", "20131231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteUpdateQuery(cmd, "20140101", "20141231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteUpdateQuery(cmd, "20150101", "20151231");
                        }
                        transaction.Commit();
                    }
            ...
            for (int i = 0; i < 100; i++)
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteSelectQuery(cmd, "20110101", "20111231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteSelectQuery(cmd, "20120101", "20121231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteSelectQuery(cmd, "20130101", "20131231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteSelectQuery(cmd, "20140101", "20141231");
                        }
                        if (rnd.NextDouble() < 0.5)
                        {
                            ExecuteSelectQuery(cmd, "20150101", "20151231");
                        }
                        transaction.Commit();
                    }
                    catch (SqlException ex) when (ex.Number == 1205)
